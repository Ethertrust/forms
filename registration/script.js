$(document).ready(function()
    {
        ...
        $(document).on("modify", ":input", function(event, data)
        {
            ...
            else
            {
                ...
                {
                    if(event.target.id=="edit-submitted-pn")
                        $('#error2').css("display", "none");
                    if(event.target.id=="edit-submitted-fio")
                        $('#error1').css("display", "none");
                    if(event.target.id=="edit-submitted-studwork")
                        $('#error4').css("display", "none");
                    if(event.target.id=="edit-submitted-birthdate")
        ...
        $('.webform-client-form').on('submit', function(event)
        {
            ...
            var submittedstudwork= $.trim($('#edit-submitted-studwork').val());
            var submittedvash_email= $.trim($('#edit-submitted-vash-email').val());
            var submittedpn= $.trim($('#edit-submitted-pn').val());
            var submittedbirthdate= $.trim($('#edit-submitted-birthdate').val());
            var allright = true;
            ...
            $.ajax({
                ...
                data: {
                    ...
                    submittedstudwork: submittedstudwork
                },
                cache: false
            }).done(function (data) {
                console.log(data);
